...
class GameHandler:
    ...
    def run(self):
        ...
        while running:
            ...
            self.canvas.blit(surf, (mx, my))


            # self.canvas = pygame.transform.scale(self.canvas, config.SCREEN_SIZE)

            frame_tex = mgl.surf2tex(self.canvas)

            if first_loop:
                print(f'{config.SCREEN_SIZE=}')
                print(f'{config.CANVAS_SIZE=}')
                print(f'{frame_tex.size=}')
                first_loop = False
